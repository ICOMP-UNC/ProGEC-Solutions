...
void exti0_isr(void)
{
    if (exti_get_flag_status(EXTI0))
    {
        ...
        systick_interrupt_disable();
         gpio_clear(LED_PORT, LED_PIN); //prende el led, el gpio_set apaga el led
         //gpio_write_bit(LED_PORT, LED_PIN, 0); //prende el led 
         //gpio_port_write(LED_PORT, 0x0000); //prende el led 
}
